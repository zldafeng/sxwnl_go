...
var cs_xxHH = []int{116, 584, 780, 399, 378, 370, 367, 367} //行星会合周期
var xxName = []string{"地球", "水星", "金星", "火星", "木星", "土星", "天王星", "海王星", "冥王星"}
...
// 求余
func Fmod2(v, n float64) float64 {
	return math.Mod(math.Mod(v, n)+n, n)
}

//=================================角度格式化=======================================
//==================================================================================
func rad2strE(d, tim float64, ext int) string { //将弧度转为字串,ext为小数保留位数
	...
	aa := []rune("   " + strconv.FormatFloat(a, 'f', -1, 64))
	bb := []rune("0" + strconv.FormatFloat(b, 'f', -1, 64))
	cc := []rune("0" + strconv.FormatFloat(c, 'f', -1, 64))
	dd := []rune("00000" + strconv.FormatFloat(d, 'f', -1, 64))

	s += string(aa[len(aa)-3:3]) + w1
	s += string(bb[len(bb)-2:2]) + w2
	s += string(cc[len(cc)-2 : 2])

	if ext != 0 {
		s += "." + string(dd[len(dd)-ext:ext]) + w3
	}
	return s
}

func rad2str(d, tim float64) string {
	return rad2strE(d, tim, 2)
} //将弧度转为字串,保留2位

func rad2str2(d float64) string { //将弧度转为字串,精确到分
	//输出格式示例: -23°59'
	var s = "+"
	var w1 = "°"
	var w2 = "'"
...
func m2fm(v float64, fx int, fs int) string { //秒转为分秒,fx为小数点位数,fs为1转为"分秒"格式否则转为"角度分秒"格式
	var gn = ""
	if v < 0 {
		v = -v
		gn = "-"
	...
	toFixed := "%." + strconv.Itoa(fx) + "f"
	if fs == 0 {
		return gn + strconv.FormatFloat(f, 'f', -1, 64) + "'" + fmt.Sprintf(toFixed, m) + `"`
	}
	if fs == 1 {
		return gn + strconv.FormatFloat(f, 'f', -1, 64) + "分" + fmt.Sprintf(toFixed, m) + "秒"
	}
	if fs == 2 {
		return gn + strconv.FormatFloat(f, 'f', -1, 64) + "m" + fmt.Sprintf(toFixed, m) + "s"
	}
	return ""
}
...
//球面转直角坐标;
func llr2xyz(JW Vector3) Vector3 {
	J := JW.X
	W := JW.Y
	R := JW.Z
	return Vector3{
...
// 直角坐标转球
func xyz2llr(xyz Vector3) Vector3 {
	x := xyz.X
	y := xyz.Y
	z := xyz.Z
	...
	r.Z = math.Sqrt(x*x + y*y + z*z)
	r.Y = math.Asin(z / r.Z)
	r.X = rad2mrad(math.Atan2(y, x))

	return r
}

// 球面坐标旋转
func llrConv(JW Vector3, E float64) Vector3 {
	J := JW.X
	W := JW.Y

	r := Vector3{}

	r.X = math.Atan2(math.Sin(J)*math.Cos(E)-math.Tan(W)*math.Sin(E), math.Cos(J))
	r.Y = math.Asin(math.Cos(E)*math.Sin(W) + math.Sin(E)*math.Cos(W)*math.Sin(J))
	r.Z = JW.Z
	r.X = rad2mrad(r.X)
...
//赤道坐标转为地平坐标
func CD2DP(z Vector3, L, fa, gst float64) Vector3 {
	a := Vector3{
		X: z.X + math.Pi/2 - gst - L,
		Y: z.Y,
		Z: z.Z,
	}
	a = llrConv(a, math.Pi/2-fa)
	a.X = rad2mrad(math.Pi/2 - a.X)
	return a
}
...
//二次曲线外推
func dt_ext(y float64, jsd int) float64 {
	dy := (y - 1820) / 100
	return -20.0 + float64(jsd)*dy*dy
}

//计算世界时与原子时之差,传入年
func dt_calc(y float64) float64 {
	dt_at_length := len(dt_at)

	y0 := dt_at[dt_at_length-2] //表中最后一年
	t0 := dt_at[dt_at_length-1] //表中最后一年的deltatT
	if y >= y0 {
...
//日期元件
func JD(y, m, d float64) float64 { //公历转儒略日
	var n = 0.00
	var G = 0
...
func DD(jd float64) Time { //儒略日数转公历
	var r Time
	var D = Int2(jd + 0.5)
	var F = jd + 0.5 - D
	var c float64 //取得日数的整数部份A及小数部分F
	if D >= 2299161 {
...
func DD2str(r Time) string { //日期转为串
	var Y = []rune("     " + strconv.FormatFloat(r.Y, 'f', -1, 64))
	var M = []rune("0" + strconv.FormatFloat(r.M, 'f', -1, 64))
	var D = []rune("0" + strconv.FormatFloat(r.D, 'f', -1, 64))
	var h = r.h
	var m = r.m
	var s = Int2(r.s + .5)
	if s >= 60 {
	...
	hh := []rune("0" + strconv.FormatFloat(h, 'f', -1, 64))
	mm := []rune("0" + strconv.FormatFloat(m, 'f', -1, 64))
	ss := []rune("0" + strconv.FormatFloat(s, 'f', -1, 64))
	YY := string(Y[len(Y)-5 : 5])
	MM := string(M[len(M)-2 : 2])
	DD := string(D[len(D)-2 : 2])
	hhh := string(hh[len(hh)-2 : 2])
	mmm := string(mm[len(mm)-2 : 2])
	sss := string(ss[len(ss)-2 : 2])
	return YY + "-" + MM + "-" + DD + " " + hhh + ":" + mmm + ":" + sss
}
func JD2str(jd float64) string { //JD转为串
	var r = DD(jd)
	return DD2str(r)
}

func toJD(t Time) float64 {
	return JD(t.Y, t.M, t.D+((t.s/60+t.m)/60+t.h)/24)
} //公历转儒略日
func setFromJD(jd float64) Time {
	return DD(jd)
} //儒略日数转公历

func timeStr(jd float64) string { //提取jd中的时间(去除日期)
	var h, m, s float64
	jd += 0.5
	jd = jd - Int2(jd)
	s = Int2(jd*86400 + 0.5)
	...
	mm := []rune("0" + strconv.FormatFloat(m, 'f', -1, 64))
	ss := []rune("0" + strconv.FormatFloat(s, 'f', -1, 64))
	return string(hh[len(hh)-2:2]) + ":" + string(mm[len(mm)-2:2]) + ":" + string(ss[len(ss)-2:2])
}

//星期相关
//Weeks: new Array('日', '一', '二', '三', '四', '五', '六', '七'),
func getWeek(jd float64) int {
	return  int(Int2(jd + 1.5 + 7000000)) % 7
} //星期计算
func nnweek(y, m, n, w float64) float64{ //求y年m月的第n个星期w的儒略日数
	var jd = JD(y, m, 1.5)           //月首儒略日
	var w0 = (int(jd) + 1 + 7000000) % 7 //月首的星期
	var r = jd - float64(w0) + 7*n + w       //jd-w0+7*n是和n个星期0,起算下本月第一行的星期日(可能落在上一月)。加w后为第n个星期w
	if w >= float64(w0) {
...
func prece(t float64, sc, mx string) float64 {
	...
	isc := strings.Index("fi w  P  Q  E  x  pi II p  th Z  z ", sc+" ") / 3
	for i := 0; i < n; i++ {
		tn = t
		c += p[isc*n+i] * tn
	}
...
//=================================岁差旋转=========================================
//J2000赤道转Date赤道
func CDllr_J2D(t float64, llr Vector3, mx string) Vector3 {
	Z := prece(t, "Z", mx) + llr.X
	z := prece(t, "z", mx)
	th := prece(t, "th", mx)
	cosW := math.Cos(llr.Y)
	cosH := math.Cos(th)
	...
	return Vector3{
		X: rad2mrad(math.Atan2(A, B) + z),
		Y: math.Asin(C),
		Z: llr.Z}
}

//Date赤道转J2000赤道
func CDllr_D2J(t float64, llr Vector3, mx string) Vector3 {
	Z := -prece(t, "z", mx) + llr.X
	z := -prece(t, "Z", mx)
	th := -prece(t, "th", mx)
	cosW := math.Cos(llr.Y)
	...
	B := cosH*cosW*math.Cos(Z) - sinH*sinW
	C := sinH*cosW*math.Cos(Z) + cosH*sinW
	return Vector3{rad2mrad(math.Atan2(A, B) + z), math.Asin(C), llr.Z}
}

//黄道球面坐标_J2000转Date分点,t为儒略世纪数
func HDllr_J2D(t float64, llr Vector3, mx string) Vector3 {
	//J2000黄道旋转到Date黄道(球面对球面),也可直接由利用球面旋转函数计算,但交角接近为0时精度很低
	r := Vector3{llr.X, llr.Y, llr.Z}
	r.X += prece(t, "fi", mx)
...
func HDllr_D2J(t float64, llr Vector3, mx string) Vector3 {
	r := Vector3{llr.X, llr.Y, llr.Z}
	r = llrConv(r, prece(t, "E", mx))
	r.X += prece(t, "x", mx)
	r = llrConv(r, -prece(t, "w", mx))
...
//=================================章动计算=========================================
//==================================================================================
var nuTab = []float64{ //IAU2000B章动序列
...
func nutation(t, zq float64) Vector2 {
	t2 := t * t
	t3 := t2 * t
	t4 := t3 * t //t的二、三、四次方
	...
	dL /= 10000000 * rad
	dE /= 10000000 * rad
	return Vector2{dL, dE} //返回IAU2000B章动值, dL是黄经章动,dE是交角章动
}

//本函数计算赤经章动及赤纬章动
func CDnutation(z Vector3, E, dL, dE float64) Vector3 {
	r := Vector3{z.X, z.Y, z.Z}
...
func nutation2(t float64) Vector2 { //中精度章动计算,t是世纪数
	var c, a float64
	t2 := t * t
	B := nutB
	dL := 0.00
	...
	return Vector2{dL / 100 / rad, dE / 100 / rad} //黄经章动,交角章动
}

func nutationLon2(t float64) float64 { //只计算黄经章动
	var a float64
	t2 := t * t
	dL := 0.00
	B := nutB
...
func MQC(h float64) float64 {
	return 0.0002967 / math.Tan(h+0.003138/(h+0.08919))
} //大气折射,h是真高度
func MQC2(ho float64) float64 {
	return -0.0002909 / math.Tan(ho+0.002227/(ho+0.07679))
} //大气折射,ho是视高度

//=================================视差改正=========================================
//==================================================================================
func parallax(z Vector3, H, fa, high float64) { //视差修正
...
var XL1_0_2 = []float64{0.00001200, 1.041, -2.3012, 1.52, 25, -0.1, 0.00000170, 0.31, -0.711, 0, 0, 0}

var XL1_0 = [][]float64{XL1_0_0, XL1_0_1, XL1_0_2}
...
var XL1_1 = [][]float64{XL1_1_0, XL1_1_1, XL1_1_2}
...
var XL1_2 = [][]float64{XL1_2_0, XL1_2_1, XL1_2_2}

var XL1 = [][][]float64{XL1_0, XL1_1, XL1_2}
...
func XL0_calc(xt int, zn int, t float64, n float64) float64 {
	t /= 10 //转为儒略千年数
	v := 0.00
	tn := 1.00
	c := 0.00
...
// TODO 返回冥王星J2000直角坐标(未实现)
func pluto_coord(t float64) Vector3 {
	/*long double c0 = PI / 180 / 100000;
	long double x = -1 + 2 * (t * 36525 + 1825394.5) / 2185000;
	long double T = t / 100000000;*/
	var r Vector3

	/*for (int i = 0; i < 9; i++) {
		long double ob = XL0Pluto[i], N = ob.length, v = 0;
		for (int j = 0; j < N; j += 3) v += ob[j] * Math.sin(ob[j + 1] * T + ob[j + 2] * c0);
		if (i % 3 == 1) v *= x;
		if (i % 3 == 2) v *= x*x;
...
func p_coord(xt int, t, n1, n2, n3 float64) Vector3 { //xt星体,T儒略世纪数,TD
	var z Vector3
	if xt < 8 {
...
func e_coord(t, n1, n2, n3 float64) Vector3 {
	var re Vector3
	re.X = XL0_calc(0, 0, t, n1)
	re.Y = XL0_calc(0, 1, t, n2)
	re.Z = XL0_calc(0, 2, t, n3)
	return re
}

//=================================月亮星历--=======================================
//==================================================================================
func XL1_calc(zn int, t float64, n float64) float64 { //计算月亮
	var ob = XL1[zn]
	var v = 0.00
	var tn = 1.00
	var t2 = t * t
...
func m_coord(t float64, n1, n2, n3 float64) Vector3 {
	var re Vector3
	re.X = XL1_calc(0, t, n1)
	re.Y = XL1_calc(1, t, n2)
	re.Z = XL1_calc(2, t, n3)
	return re
}
...
func gxc_sunLon(t float64) float64 { //太阳光行差,t是世纪数
...
func pGST(T, dt float64) float64 { //传入T是2000年首起算的日数(UT),dt是deltatT(日),精度要求不高时dt可取值为0
...
func pGST2(jd float64) float64 { //传入力学时J2000起算日数，返回平恒星时
	var dt = dt_T(jd)
	return pGST(jd-dt, dt)
}

func sunShengJ(jd, L, fa, sj float64) float64 { //太阳升降计算。jd儒略日(须接近L当地平午UT)，L地理经度，fa地理纬度，sj=-1升,sj=1降
	jd = math.Floor(jd+0.5) - L/pi2
	for i := 0; i < 2; i++ {
		var T = jd / 36525
		var E = (84381.4060 - 46.836769*T) / rad        //黄赤交角
		var t = T + (32*(T+1.8)*(T+1.8)-20)/86400/36525 //儒略世纪年数,力学时
		var J = (48950621.66 + 6283319653.318*t + 53*t*t - 994 + 334166*math.Cos(4.669257+628.307585*t) + 3489*math.Cos(4.6261+1256.61517*t) + 2060.6*math.Cos(2.67823+628.307585*t)*t) / 10000000
		var sinJ = math.Sin(J)
		var cosJ = math.Cos(J)                                                                                 //太阳黄经以及它的正余弦值
		var gst = (0.7790572732640+1.00273781191135448*jd)*pi2 + (0.014506+4612.15739966*T+1.39667721*T*T)/rad //恒星时(子午圈位置)
		var A = math.Atan2(sinJ*math.Cos(E), cosJ)                                                             //太阳赤经
		var D = math.Asin(math.Sin(E) * sinJ)                                                                  //太阳赤纬
		var cosH0 = (math.Sin(-50*60/rad) - math.Sin(fa)*math.Sin(D)) / (math.Cos(fa) * math.Cos(D))
		if math.Abs(cosH0) >= 1 {
			return 0
		}                                                    //太阳在地平线上的cos(时角)计算
		jd += rad2rrad(sj*math.Acos(cosH0)-(gst+L-A)) / 6.28 //(升降时角-太阳时角)/太阳速度
	}
	return jd //反回格林尼治UT
}

func pty_zty(t float64) float64 { //时差计算(高精度),t力学时儒略世纪数
	var t2 = t * t
	var t3 = t2 * t
	var t4 = t3 * t
	var t5 = t4 * t
	...
	//地球坐标
	z.X = XL0_calc(0, 0, t, 50) + math.Pi + gxc_sunLon(t) + dL
	z.Y = -(2796*math.Cos(3.1987+8433.46616*t) + 1016*math.Cos(5.4225+550.75532*t) + 804*math.Cos(3.88+522.3694*t)) / 1000000000
...
func M_Lon_t(W float64) float64 { //已知真月球黄经求时间
	var t float64
...
func MS_aLon_t(W float64) float64 { //已知月日视黄经差求时间
	var t float64
...
func S_aLon_t(W float64) float64 { //已知太阳视黄经反求时间
	var t float64
	var v = 628.3319653318
 ...
 ****/
func MS_aLon_t2(W float64) float64 { //已知月日视黄经差求时间,高速低精度,误差不超过600秒(只验算了几千年)
	var t float64
	var v = 7771.37714500204
	t = (W + 1.08472) / v
	var L float64
...
func S_aLon_t2(W float64) float64 { //已知太阳视黄经反求时间,高速低精度,最大误差不超过600秒
	var t float64
	var v = 628.3319653318
	t = (W - 1.75347 - math.Pi) / v
...
func moonIll(t float64) float64 { //月亮被照亮部分的比例
	var t2 = t * t
	var t3 = t2 * t
	var t4 = t3 * t
	var D, M, m, a float64
...
func moonRad(r, h float64) float64 { //转入地平纬度及地月质心距离,返回站心视半径(角秒)
	return cs_sMoon / r * (1 + math.Sin(h)*cs_rEar/r)
}

func moonMinR(t, min float64) Vector2 { //求月亮近点时间和距离,t为儒略世纪数力学时
...
func moonNode(t, asc float64) Vector2 { //月亮升交点
	...
	w = XL1_calc(1, t, -1)
	t -= w / v
	var re = Vector2{t, XL1_calc(0, t, -1)}
	return re
}
func earthMinR(t, min float64) Vector2 { //地球近远点
