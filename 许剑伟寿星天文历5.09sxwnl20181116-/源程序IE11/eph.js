...
var rsGS = {
    ...
    jieX3: function (jd) { //界线表
        ...
        for (i = 0; i < N; i++, t += dt) {
            ...
            if (p[1] != 100) s2 += rad2str2(p[0]) + ' ' + rad2str2(p[1]) + '|', k++; else s2 += '-------------------|'; //半影北界
            p = this.nanbei(M, vx, vy, +1, B.r2, I);
            if (p[1] != 100) s2 += rad2str2(p[0]) + ' ' + rad2str2(p[1]) + '|', k++; else s2 += '-------------------|'; //本影北界
            p = this.bseXY2db(M[0], M[1], I, 1);
            if (p[1] != 100) s2 += rad2str2(p[0]) + ' ' + rad2str2(p[1]) + '|', k++; else s2 += '-------------------|'; //中心线
            p = this.nanbei(M, vx, vy, -1, B.r2, I);
            if (p[1] != 100) s2 += rad2str2(p[0]) + ' ' + rad2str2(p[1]) + '|', k++; else s2 += '-------------------|'; //本影南界
            p = this.nanbei(M, vx, vy, -1, r, I);
            if (p[1] != 100) s2 += rad2str2(p[0]) + ' ' + rad2str2(p[1]) + ' ', k++; else s2 += '------------------- '; //半影南界
            if (k) s += s2 + '<br>';
        }
